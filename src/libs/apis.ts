...
export async function getFeaturedRoom() {
    const result = await sanityClient.fetch<Room>(
        ...
        // since we are in developpment and we don't want our app fetch data every 30 min because our app doesnt change this often or we don't want to see our changes happening,
        //  we won't use: {next: {revalidate: 1800 }})// 30
        // but instead the following so that this function get called on every request
        { cache: 'no-cache' }
    );  
       return result;   
